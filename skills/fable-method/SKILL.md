...
**Tools you'll need:** `fs.list`, `fs.find`, `fs.read`, `fs.grep` (orient), `code.run`/`test.run` (verify), `specialist.delegate` (act), `verify.score` (quality gate). Load via `tools.load` if not available.
