...
How a skill is reached — and the two loads you pay for the choice. In this orchestrator every discovered skill is model-invoked (its description is mandatory and sits in the catalog); "user-invoked" exists only as a forced-load pointer or slash command layered on top.

### Model-Invoked

A skill whose **description** sits in the always-injected catalog, so the agent can fire it autonomously via `skill.load` — and other skills can reach it the same way. Pays a permanent **context load** on every turn in exchange for that discoverability. A model-invoked skill whose content is all **reference** is also one home for shared reference: another skill can invoke it, so reference needed by several skills lives in one place. Here, every skill is model-invoked — so the real choice is whether a skill earns its catalog slot at all.
...
The skill's machine-readable trigger, and the one **context pointer** every skill here is forced to keep loaded at all times (the catalog test rejects blank descriptions). The source of every skill's **context load** — prune it hardest.

_Avoid_: frontmatter, summary

### Context Pointer

A reference held in the agent's context that names some out-of-context material and encodes the condition for reaching it. The **description** is the top-level context pointer (catalog → skill body); the `files` map returned by `skill.load` holds the pointers to disclosed sibling files one level down. Its wording, not the target, decides _when_ the agent reaches — and _how reliably_. A must-have target behind a weakly worded pointer is a variance bug: fix the wording first, and inline the material only if sharpening fails.

_Avoid_: link, reference, import

### Context Load

The cost a skill imposes on the agent's context window — its **description**, always loaded, spending both tokens and attention; plus every loaded body, which stays in context for the rest of the run (why the catalog forbids bulk-loading skills). The brake on splitting into more skills.

_Avoid_: token cost, context bloat

### Cognitive Load

The cost a user-invoked mechanism imposes on the human — what they must hold in their head: which toggles and slash commands exist and when to reach for each (the human is the index). Not a cost to minimise: it is the price of human agency. Spend it where human judgement matters; remove it where it does not.

_Avoid_: human index, burden, overhead

### Router Skill

A skill whose job is to point at other skills — naming each and when to reach for it — so there is one thing to remember instead of many. Here a router can be model-invoked (its description names the triggers of what it routes to), which makes it strictly more capable than a classic user-only router: the agent can fire the router itself.

_Avoid_: dispatcher, menu, registry, index, router procedure

### Granularity

How finely you divide skills. Finer division spends one of the two loads: more skills spend **context load** (more descriptions crowding the catalog and competing for attention); more forced-load commands spend **cognitive load** (more for the human to remember). Two cuts guide the division. By **invocation**, split off a skill where you have a distinct **leading word** to trigger it — a trigger word you actually use in your prompts. By **sequence**, split a run of **steps** where a step's **post-completion steps** need hiding, since isolating it in its own context clears what follows. Beware the reverse: merging sequences exposes each step's post-completion steps to what follows, inviting premature completion.
...
A skill with no **steps** uses just the bottom two rungs — often a legitimately flat peer-set (e.g. every rule of a review on one rung), which is a fine arrangement, not a smell. The hierarchy is independent of invocation: a skill can be all steps, all reference, or both. When a skill has steps, in-file reference that should be disclosed buries them and turns attending to them into a coin-flip — a variance lever, not just a legibility one. Keep the top of the ladder legible; push down it whatever you can.

_Avoid_: structure, organization, layout

### Steps

The ordered actions the agent performs — when a skill has them, the primary tier of its content, and the part that earns its place in SKILL.md. Not every skill has steps: a skill can be all steps, all **reference**, or both. Every step ends on a **completion criterion**, clear or vague.
...
**Reference** that lives outside the skill system — a plain file, no **description**, not invocable — that any skill can point at (e.g. this repo's `docs/`, a project's `AGENTS.md`). The home for shared reference that needn't fire on its own.
