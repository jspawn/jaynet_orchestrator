...
Create `.jspace/WORKSPACE.md` in your workspace at the start of any task that will span
stages, files, or turns, then fill `Goal` and `Next` before the first seam. If there is no
filesystem here, keep the five sections in the conversation and restate them at each seam —
the file was never the point, the re-reading was.
...
Add at most two live `Core` entries as `name — defining fact`; number `Verified` and `Open`
entries yourself (`✓01`, `?01`, …) so their identifiers remain stable.
