#!/usr/bin/env bash
# Launch the orchestrator's SECOND local "brain" on :8091 / GPU 1 — the parallel
# instance that lets a fan-out of independent sub-agents overlap across both cards.
#
...
PORT=8091                                    # brain2; brain1=8090, litellm=4000, rag=8095/8096
HOST="${HOST:-127.0.0.1}"
...
# ROCR_VISIBLE_DEVICES double-filters: ROCR_VISIBLE_DEVICES=1 exposes only
# physical GPU 1, renumbered to ROCr-index 0, and then HIP_VISIBLE_DEVICES=1
# asks for index 1 in that one-device list — out of range, so it mis-selects
# (model lands on CPU / the wrong card, GPU 1 stays idle). HIP alone enumerates
# all cards and picks index N correctly — the same pattern start-coder.sh uses
# to pin the coder to GPU 1.
if [[ -n "$ORCH_GPU" ]]; then
...
echo "-------------------------------------------------------"
echo "  ORCHESTRATOR BRAIN 2 (llama-server)"
