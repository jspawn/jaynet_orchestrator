...
| brain-moe *(example)* | **Qwen3-30B-A3B-Instruct-2507** | same brain slot, swap-in | 8090 | one 24 GB GPU |
| specialist *(example)* | **Qwen2.5-Coder-32B-Instruct** (dense) | LiteLLM alias `local-specialist` | 8080 | one 24 GB GPU |
...
The two *(example)* presets ship without model files — they are the
teaching pair for the classic two-GPU layout: a **MoE brain** (30B total but
...
- **Brain (MoE):** Qwen3-30B-A3B-Instruct-2507 — Apache-2.0 (the shipped
  `brain-moe` example preset)
- **Specialist (dense, coding):** Qwen2.5-Coder-32B-Instruct — Apache-2.0
  (the shipped `specialist` example preset)
