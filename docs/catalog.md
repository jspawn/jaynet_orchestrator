...
| `coupled-systems` | Coordinate a task whose parts depend on each other — a migration, a cross-cutting refactor, a plan or schedule with shared resources, a multi-component design. Load when a locally correct change can s… |
...
| `project-charter` | Charter interview for a new project: ask one question at a time (with your recommended answer) and compile the answers into the project's LLM wiki as its first charter pages (overview, goals, constrai… |
| `research-and-verify` | Find a specific fact, source, URL, version, or dataset where the answer must be RIGHT and is judged against ground truth (typical shape: "find the official X and write it to file Y"). Load when the ta… |
