...
| `REASONING_BUDGET` | `--reasoning-budget` | Cap on THINKING tokens — llama.cpp force-closes the think block at the budget, reserving the rest of the completion cap (`orchestrator.sampling.max_tokens`, default 8192) for the actual answer. Uncapped thinking can otherwise eat the whole cap and return an empty answer. ~half the completion cap is the sweet spot; empty = unlimited. **Needs a recent llama.cpp build** (b10380+ verified; older builds fail to start on the unknown flag — check `llama-server --help` or clear the key). **Only works when llama.cpp detects the template's think tags** — check `/props` on the running server: `reasoning_format: none` means the budget is a silent no-op (custom think tags, e.g. K2-Horizon's `<ifm|think>`); use `REASONING_EFFORT` instead. |
| `REASONING_EFFORT` | `--reasoning-effort` | Template-level think mode (`minimal`/`low`/`medium`/`high`/`xhigh`); requires a chat template with `supports_reasoning_effort` (check `/props` → `chat_template_caps`). The lever for models whose custom think tags make `REASONING_BUDGET` inert — e.g. K2-Horizon maps `medium` → `<ifm|think_fast>`. Empty = template default (usually `high`). |
...
2. **Admin → Presets → + new preset** — fill the row fields and the launch
   ...
   **Browse model files…** button opens the models-dir inventory (GGUF and
   template files by default, **show all** reveals the rest; folders start
   collapsed and expand individually) with the
