...
rubric, and deterministic checks (`answer_exact_any` GAIA-style exact
match, an optional Python `checker` script run inside the sandbox after the
last turn) override it where answers are knowable. Cases can bind projects
...
with `code.run`/`code.execute` routed inside it; missing podman or image skips the
