...
workspace — relative file writes land in `exec-work/` and survive across
calls within the run — and output that overflows the inline caps spills to
a file instead of being dropped. In benchmark (container) runs it executes
...
with `code.execute` routed inside it; missing podman or image skips the
