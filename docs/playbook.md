...
through v1.8.0 (devbox containers, strength routing with real swaps, the
loop's enforcement machinery — routing nudge, stall ladder, deliverable
check, procedure checkpoints — the merged `code.run`, `/loop` + `| check:`,
guided start, connector packages, judge calibration, benchmark skill A/Bs);
every claim here was checked against the implementations, not just the
descriptions.
...
- **Procedures** — the `shape:`-tagged family (`implement-from-spec`,
  ...
  *process* for a task shape, born from eval failure clusters. A confident
  ...
  enforced by the loop — appended to stall-ladder rungs, nudged once before
  a final answer is accepted. The checkpoint that flipped a benchmark case
  from fail to pass was "run the spec's OWN tests" — the step small models
  skip.
...
The same tool with `language=python` is the snippet scratchpad: short
Python in a firejail sandbox, no network, with `llm_query` subcalls
available when the run grants them (see the delegation section). It has a
persistent per-run workspace — relative file writes land in `exec-work/`
and survive across calls within the run — and output that overflows the
inline caps spills to a file instead of being dropped. In benchmark
(container) runs both languages execute
inside the case's podman container instead, with `language: bash` for the
...
- **`llm_query` subcalls** — the RLM primitive ([Recursive Language
  ...
  from *inside* a `code.run` python snippet, over a per-run unix socket (the
...
behaviour). Grading has two layers: the judge model scores against the
rubric (shipped default `local-specialist` — a cloud judge is an explicit
override, and its own accuracy is measurable: the *Judge calibration*
button grades ten frozen transcripts with known verdicts and reports
agreement), and deterministic checks (`answer_exact_any` GAIA-style exact
match, an optional Python `checker` script run inside the sandbox after the
last turn, and `canary_not_in_tool_args` — a planted string that must never
appear in any tool call's arguments, scanned from the trace; the
prompt-injection cases) override it where answers are knowable. Cases can
bind projects
(fixture files, seeded or generated by `seed_code`) or a **podman
container** — the case then runs against the container's real environment
with `code.run` routed inside it; missing podman or image skips the
...
run). The **Benchmark** sub-tab runs one suite under N variants
(model × sampling × reps, harness `full` vs `brain`-only) and compares
pass rates per label — and a variant can run **without named skills**
(`disabled_skills`: hidden from the catalog, refused by `skill.load`), so
"does this skill actually earn its context?" becomes an A/B with a number
attached. `trace.*` reads the run history back
...
- **The eval harness grew teeth.** Cases can bind projects (fixture files
  ...
  cleanly when an install lacks the tools — or a container backend: cases
  can run inside per-task podman containers with deterministic graders
  (exact-match keys, checker scripts) doing the scoring. Standardized
  benchmarks are one `bench.import` away. Doctrines are guarded by
...
- **The youngest pieces are still proving themselves.** j-space shipped
  ...
  code tool are newer still. The proof that the j-space gate
...
- **Staleness detection covers both write paths now.** Web-API edits *and*
  ...
  projects root). Still untracked: files a `code.run` script writes
...
- **A plugin by other hands.** benchlab is the second shipped plugin, but
  ...
  interface is still a plugin the core authors didn't write (parked in
  ToDos).
