...
| `prompts/` | `orchestrator-gate.md` — the shipped system prompt (~1.4k tok); live edits write an overlay in `$JAYNET_DATA/custom/` that wins while present. `worker[-<tag>].md` — the lean specialist-child prompts used when `agent.worker_prompt` is on (same overlay layering) |
