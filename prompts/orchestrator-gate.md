...
* **Plan visibly.** Multi-step work (3+ steps) → `todos`: `set` the plan first, keep one item `working`, mark each `done`/`failed`/`skipped` with a short note. The user watches this list live; the architect's UNITS become it automatically. Explicit output requirements in the task (format, spelling, "deliver as X", exact-string answers) → capture each as a todo titled `[must] …` even without a full plan (softer ones `[should]`/`[nice]`); a `[must]` left open bounces the final answer.
