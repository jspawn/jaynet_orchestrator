...
/* ---- edit a fired prompt / retry an empty one ------------------------------
   Both drop the turn AND everything after it, put the prompt back in the
   composer; retry also resends immediately. Turns are client-owned and pushed
   wholesale (POST /api/chats + PUT /api/current-chat), so truncation here is
   authoritative everywhere after persistChat(). */
async function truncateTo(i, resend){
  if(currentRun){ setStatus("stop the run first", true); return; }
  const t=chat.turns[i]; if(!t) return;
  const after=chat.turns.length-i-1;
  if(after>0 && !(await dlgConfirm(
      "Editing drops the "+after+" later turn"+(after>1?"s":"")+
      " — the chat continues from your edited prompt."))) return;
  chat.turns.length=i;
  renderChatTurns(); persistChat(); syncIfSaved();
...
function applyEvent(c, ev){
  ...
  switch(ev.type){
    ...
    case "fresh_retry":
      ...
      break;
    case "markup_leak":
      warnRow(c, "<span class='cn warn'>markup leak</span> <span class='meta'>"+
                 "answer was leaked tool-call markup — restating in plain text</span>");
      break;
    case "requirements_gate":
      warnRow(c, "<span class='cn warn'>requirements</span> <span class='meta'>"+
                 "final answer bounced: "+esc_html((d.open||[]).join("; "))+"</span>");
      break;
    case "budget_warning":
...
// runs and the admin view is structural (no message/tool content) unless the
// user explicitly opts into sharing private context — so this is safe to use
// even for private chats.
$("#flagBtn").onclick=()=>{
  const rids=chat.turns.map(t=>t.run_id).filter(Boolean);
  if(!rids.length){ setStatus("nothing to flag yet", false); return; }
  $("#flagComment").value=""; $("#flagPrivate").checked=false;
  $("#flagModal").classList.add("show");
};
$("#flagNo").onclick=()=>$("#flagModal").classList.remove("show");
$("#flagYes").onclick=async()=>{
  $("#flagModal").classList.remove("show");
  const rids=chat.turns.map(t=>t.run_id).filter(Boolean);
  if(!rids.length) return;
  const comment=$("#flagComment").value.trim();
  const include_private=$("#flagPrivate").checked;
  const r=await api("/api/flag",{method:"POST",headers:{"content-type":"application/json"},
    body:JSON.stringify({comment, include_private, conversation_id:chat.cid, chat_title:chat.title, run_ids:rids})});
...
function openStream(runId){
  ...
  es.addEventListener("run_finish", onEv(ev=>{
    ...
    finalize(cur, ev.data, chat.turns.length);
    if(pending){
      if(ev.data.compact){
